...
def scrape_poet_poems(slug: str, max_poems: int = 50) -> List[dict]:
    ...
    for link in poem_links:
        href = link['href']
        # Aldiwan poem links usually contain 'poem' and end with '.html'
        # e.g., 'poem71682.html' or '/poem71682.html'
    ...
    for i, url in enumerate(poem_urls[:max_poems]):
        try:
            ...
            poem_data = parser.parse_poem(soup, url)
            if poem_data and poem_data.get('verses'):
                # Validate with Poem model
                poem_obj = Poem.from_raw_data(poem_data, url)
                if poem_obj.validate():
                    poem_dict = poem_obj.to_dict()
                    # Add poet slug for ingestion
                    poem_dict['poet_slug'] = slug
                    scraped_poems.append(poem_dict)
                    logger.info(f"  [{i+1}/{max_poems}] Scraped: {poem_data['title']}")
                else:
                    logger.warning(f"  [{i+1}/{max_poems}] Validation failed: {url}")
            else:
